          ...
          {items.map((it, idx) => (
            <div key={it} className="flex-1 w-[0] flex flex-col justify-start items-center gap-2">
              {errorCurrent && idx + 1 === errorCurrent
                ...
                : <div className={classNames('w-[12px] h-[12px] rounded-xl', current >= idx ? 'gradient-border1' : 'bg-[#708077]')} />}
              <div className={classNames('text-[16px] text-center', current >= idx ? 'gradient-text1' : 'text-[#708077]')}>{it}</div>
            </div>
          ))}
        </div>
      </div>
